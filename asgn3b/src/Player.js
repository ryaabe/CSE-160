...
class Player {
  ...
  constructor() {
    ...
    this.moveSpeed = 2.20; // player move speed
    ...
    this.freeCamSpeed = 12.0;
    this.freeCamVerticalAxis = 0;
  }
  ...
  setFreeCamVerticalAxis(axis) {
    const numericAxis = Number(axis);
    if (!Number.isFinite(numericAxis)) {
      this.freeCamVerticalAxis = 0;
      return;
    }
    this.freeCamVerticalAxis = Math.max(-1, Math.min(1, numericAxis));
  }

  setFreeCamEnabled(enabled) {
    const nextEnabled = !!enabled;
    if (this.isFreeCam === nextEnabled) return this.isFreeCam;

    this.isFreeCam = nextEnabled;
    const velocity = this.playerVelocity.elements;
    velocity[0] = 0;
    velocity[1] = 0;
    velocity[2] = 0;
    this.isGrounded = false;
  ...
  toggleFreeCam() {
    return this.setFreeCamEnabled(!this.isFreeCam);
  }
  ...
  moveFreeCam(dt) {
    ...
    const wishInputX = this.wishDir.elements[0];
    const wishInputZ = this.wishDir.elements[2];
    const verticalInput = this.freeCamVerticalAxis;

    let moveX = 0;
    let moveY = 0;
    let moveZ = 0;

    if (wishInputX !== 0 || wishInputZ !== 0) {
      const wishInputMagnitude = Math.hypot(wishInputX, wishInputZ);
      if (wishInputMagnitude > 0) {
        ...
        moveX += right[0] * localWishX + forward[0] * (-localWishZ);
        moveY += right[1] * localWishX + forward[1] * (-localWishZ);
        moveZ += right[2] * localWishX + forward[2] * (-localWishZ);
      }
    }

    if (verticalInput !== 0) {
      const worldUp = this.worldUp.elements;
      moveX += worldUp[0] * verticalInput;
      moveY += worldUp[1] * verticalInput;
      moveZ += worldUp[2] * verticalInput;
    }
