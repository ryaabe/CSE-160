...
var FSHADER_SOURCE = `
  ...
  uniform float u_UseFrontFaceOnly;
  ...
  void main() {
    ...
    if (u_whichTexture == -2) {
      ...
      color.a *= clamp(u_FragColor.a, 0.0, 1.0);

    } else {
      color = vec4(1,.2,.2,1);
    }

    if (u_UseFrontFaceOnly > 0.5 && !gl_FrontFacing) {
      discard;
    }
...
class Renderer {
  ...
  constructor(canvas, gl) {
    ...
    this.infectionOverlayBuffer = null;
    this.infectionOverlayCapacity = 0;
    this.infectionOverlayCount = 0;
    this.infectionOverlaySyncedRevision = -1;
    this.dreadChaseOverlayBuffer = null;
    this.dreadChaseOverlayCapacity = 0;
    this.dreadChaseOverlayCount = 0;
    this.dreadChaseOverlaySyncedRevision = -1;
    this.infectionParticleFaceVisibility = {
      front: true,
      right: false,
      back: false,
      left: false,
    ...
    this.nightCycleProgressOverride = null;
    this.fogNear = 6.0;
    this.fogFar = 45.0;
    this.nightCycleEnabled = false;
  ...
  connectVariablesToGLSL() {
    ...
    this.u_UseFrontFaceOnly = gl.getUniformLocation(gl.program, "u_UseFrontFaceOnly");
    if (!this.u_UseFrontFaceOnly) {
      console.log("Failed to get the storage location of u_UseFrontFaceOnly");
      return false;
    }
    ...
    gl.uniform1f(this.u_UseFrontFaceOnly, 0.0);
  ...
  ensureInfectionOverlayBuffer(world) {
    if (!this.instancedExt) return null;
    const infectedCoords = Array.isArray(world?.infectedBlockCoords) ? world.infectedBlockCoords : [];
    const targetCount = infectedCoords.length;
    if (targetCount <= 0) {
      this.infectionOverlayCount = 0;
      this.infectionOverlaySyncedRevision = world?.infectionRevision ?? -1;
      return null;
    }

    const gl = this.gl;
    const currentRevision = world?.infectionRevision ?? targetCount;
    if (currentRevision < this.infectionOverlaySyncedRevision) {
      this.infectionOverlayCount = 0;
    }

    if (!this.infectionOverlayBuffer || this.infectionOverlayCapacity < targetCount) {
      const nextCapacity = Math.max(targetCount, Math.max(256, this.infectionOverlayCapacity * 2));
      const nextBuffer = gl.createBuffer();
      if (!nextBuffer) return null;

      gl.bindBuffer(gl.ARRAY_BUFFER, nextBuffer);
      gl.bufferData(gl.ARRAY_BUFFER, nextCapacity * 3 * 4, gl.DYNAMIC_DRAW);
      if (this.infectionOverlayBuffer) {
        gl.deleteBuffer(this.infectionOverlayBuffer);
      }
      this.infectionOverlayBuffer = nextBuffer;
      this.infectionOverlayCapacity = nextCapacity;
      this.infectionOverlayCount = 0;
    }

    gl.bindBuffer(gl.ARRAY_BUFFER, this.infectionOverlayBuffer);

    if (targetCount < this.infectionOverlayCount) {
      this.infectionOverlayCount = 0;
    }

    if (this.infectionOverlayCount < targetCount) {
      const start = this.infectionOverlayCount;
      const appendCount = targetCount - start;
      const appendOffsets = new Float32Array(appendCount * 3);
      let writeIndex = 0;
      for (let i = start; i < targetCount; i += 1) {
        const coord = infectedCoords[i];
        appendOffsets[writeIndex++] = coord?.x ?? 0;
        appendOffsets[writeIndex++] = coord?.y ?? 0;
        appendOffsets[writeIndex++] = coord?.z ?? 0;
      }
      gl.bufferSubData(gl.ARRAY_BUFFER, start * 3 * 4, appendOffsets);
      this.infectionOverlayCount = targetCount;
    }

    this.infectionOverlaySyncedRevision = currentRevision;

    return {
      textureNum: -2,
      color: [...(world?.infectionColor || [0.02, 0.02, 0.02, 1.0])],
      uvByFace: null,
      tintByFace: null,
      instanceBuffer: this.infectionOverlayBuffer,
      instanceCount: this.infectionOverlayCount,
    };
  }

  ensureDreadChaseOverlayBuffer(world) {
    if (!this.instancedExt) return null;
    const infectedCoords = Array.isArray(world?.dreadChaseInfectedCoords) ? world.dreadChaseInfectedCoords : [];
    const targetCount = infectedCoords.length;
    const currentRevision = world?.dreadChaseInfectionRevision ?? targetCount;
    if (targetCount <= 0) {
      this.dreadChaseOverlayCount = 0;
      this.dreadChaseOverlaySyncedRevision = currentRevision;
      return null;
    }

    const gl = this.gl;
    if (!this.dreadChaseOverlayBuffer || this.dreadChaseOverlayCapacity < targetCount) {
      const nextCapacity = Math.max(targetCount, Math.max(256, this.dreadChaseOverlayCapacity * 2));
      const nextBuffer = gl.createBuffer();
      if (!nextBuffer) return null;

      gl.bindBuffer(gl.ARRAY_BUFFER, nextBuffer);
      gl.bufferData(gl.ARRAY_BUFFER, nextCapacity * 3 * 4, gl.DYNAMIC_DRAW);
      if (this.dreadChaseOverlayBuffer) {
        gl.deleteBuffer(this.dreadChaseOverlayBuffer);
      }
      this.dreadChaseOverlayBuffer = nextBuffer;
      this.dreadChaseOverlayCapacity = nextCapacity;
      this.dreadChaseOverlayCount = 0;
      this.dreadChaseOverlaySyncedRevision = -1;
    }

    gl.bindBuffer(gl.ARRAY_BUFFER, this.dreadChaseOverlayBuffer);
    if (
      this.dreadChaseOverlaySyncedRevision !== currentRevision ||
      this.dreadChaseOverlayCount !== targetCount
    ) {
      const offsets = new Float32Array(targetCount * 3);
      let writeIndex = 0;
      for (let i = 0; i < targetCount; i += 1) {
        const coord = infectedCoords[i];
        offsets[writeIndex++] = coord?.x ?? 0;
        offsets[writeIndex++] = coord?.y ?? 0;
        offsets[writeIndex++] = coord?.z ?? 0;
      }
    ...
    return {
      textureNum: -2,
      color: [...(world?.dreadChaseInfectionColor || world?.infectionColor || [0.02, 0.02, 0.02, 1.0])],
      uvByFace: null,
      tintByFace: null,
      instanceBuffer: this.dreadChaseOverlayBuffer,
      instanceCount: this.dreadChaseOverlayCount,
    };
  }

  renderInfectionOverlay(world) {
    const infectedCoords = Array.isArray(world?.infectedBlockCoords) ? world.infectedBlockCoords : [];
    if (infectedCoords.length === 0) return;

    const gl = this.gl;
    const overlayColor = world?.infectionColor || [0.02, 0.02, 0.02, 1.0];
    gl.enable(gl.POLYGON_OFFSET_FILL);
    gl.polygonOffset(-1.0, -1.0);

    if (this.useInstancedBlocks && this.instancedExt) {
      const overlayBatch = this.ensureInfectionOverlayBuffer(world);
      if (overlayBatch && overlayBatch.instanceCount > 0) {
        this.cubeMesh.renderInstanced(this, overlayBatch, -2);
      }
    ...
    for (const coord of infectedCoords) {
      if (!coord) continue;
      modelMatrix.setTranslate(coord.x, coord.y, coord.z);
      this.cubeMesh.render(this, modelMatrix, overlayColor, -2, null, null);
    }
    gl.disable(gl.POLYGON_OFFSET_FILL);
  }

  renderDreadChaseInfectionOverlay(world) {
    const infectedCoords = Array.isArray(world?.dreadChaseInfectedCoords) ? world.dreadChaseInfectedCoords : [];
    if (infectedCoords.length === 0) return;

    const gl = this.gl;
    const overlayColor = world?.dreadChaseInfectionColor || world?.infectionColor || [0.02, 0.02, 0.02, 1.0];
    gl.enable(gl.POLYGON_OFFSET_FILL);
    gl.polygonOffset(-1.0, -1.0);

    if (this.useInstancedBlocks && this.instancedExt) {
      const overlayBatch = this.ensureDreadChaseOverlayBuffer(world);
      if (overlayBatch && overlayBatch.instanceCount > 0) {
        this.cubeMesh.renderInstanced(this, overlayBatch, -2);
      }
      gl.disable(gl.POLYGON_OFFSET_FILL);
  ...
  renderInfectionParticles(world) {
    const particles = Array.isArray(world?.infectionParticles) ? world.infectionParticles : [];
    if (particles.length === 0) return;

    const playerRotation = world?.player?.transform?.rotation?.elements;
    const playerPosition = world?.player?.transform?.position?.elements;
    const yaw = playerRotation ? (Number(playerRotation[1]) || 0) : 0;
    const pitch = playerRotation ? -(Number(playerRotation[0]) || 0) : 0;
    const color = world?.infectionParticleColor || [0.0, 0.0, 0.0, 1.0];
    const modelMatrix = this.entityModelMatrix;
    const quadThickness = 0.001;

    let renderRadiusSq = Infinity;
    let playerX = 0;
    let playerY = 0;
    let playerZ = 0;
    if (playerPosition) {
      playerX = Number(playerPosition[0]) || 0;
      playerY = Number(playerPosition[1]) || 0;
      playerZ = Number(playerPosition[2]) || 0;
      const baseRadius = Math.max(0, Number(world?.infectionParticleEmitRadius) || 10);
      const renderRadius = baseRadius + 2.0;
    ...
    for (const particle of particles) {
      ...
      const size = Math.max(0.005, Number(particle.size) || 0.02);
      if (!Number.isFinite(x) || !Number.isFinite(y) || !Number.isFinite(z)) continue;

      if (Number.isFinite(renderRadiusSq)) {
        const dx = x - playerX;
        const dy = y - playerY;
        const dz = z - playerZ;
      ...
      modelMatrix.translate(-size * 0.5, -size * 0.5, 0);
      modelMatrix.scale(size, size, quadThickness);
  ...
  renderWindLeafParticles(world) {
    const particles = Array.isArray(world?.windLeafParticles) ? world.windLeafParticles : [];
    if (particles.length === 0) return;
    ...
    const yaw = playerRotation ? (Number(playerRotation[1]) || 0) : 0;
    const pitch = playerRotation ? -(Number(playerRotation[0]) || 0) : 0;
    const color = world?.windLeafParticleColor || [0.07, 0.2, 0.08, 1.0];
    const modelMatrix = this.entityModelMatrix;
    const quadThickness = 0.001;
    ...
    if (playerPosition) {
      ...
      playerY = Number(playerPosition[1]) || 0;
      playerZ = Number(playerPosition[2]) || 0;
      const baseRadius = Math.max(0, Number(world?.windLeafParticleEmitRadius) || 12);
      const renderRadius = baseRadius + 4.0;
      renderRadiusSq = renderRadius * renderRadius;
    }
    ...
    for (const particle of particles) {
      ...
      const y = Number(particle.y);
      const z = Number(particle.z);
      const size = Math.max(0.004, Number(particle.size) || 0.016);
      if (!Number.isFinite(x) || !Number.isFinite(y) || !Number.isFinite(z)) continue;
  ...
  serializeFaceVisibility(faceVisibility) {
    const faceOrder = ["front", "right", "back", "left", "top", "bottom"];
    if (!faceVisibility) return "";
    return faceOrder.map((face) => (faceVisibility[face] === false ? "0" : "1")).join("");
  }

  serializeFrontFaceOnly(frontFaceOnly) {
    return frontFaceOnly ? "F1" : "F0";
  }

  serializeUseFaceShading(useFaceShading) {
    return useFaceShading === false ? "S0" : "S1";
  }
  ...
  buildBlockBatchKey(block) {
    ...
    return [
      ...
      this.serializeFaceVisibility(block?.faceVisibility || null),
      this.serializeFrontFaceOnly(block?.frontFaceOnly === true),
      this.serializeUseFaceShading(block?.useFaceShading),
    ].join("#");
  }
    ...
    for (const block of world.blocks.values()) {
      ...
      if (!batch) {
        batch = {
          ...
          faceVisibility: block.faceVisibility || null,
          frontFaceOnly: block.frontFaceOnly === true,
          useFaceShading: block.useFaceShading !== false,
          offsets: [],
          instanceBuffer: null,
          instanceCount: 0,
        };
  ...
  setNightCycleEnabled(enabled) {
    this.nightCycleEnabled = !!enabled;
    if (!this.nightCycleEnabled) {
      this.nightCycleProgressOverride = null;
      for (let i = 0; i < 4; i += 1) {
        this.fogColor[i] = this.dayFogColor[i];
      }
    }
    return this.nightCycleEnabled;
  }

  setNightCycleProgressOverride(progress = null) {
    if (progress == null) {
      this.nightCycleProgressOverride = null;
      return this.nightCycleProgressOverride;
    }
    ...
    this.nightCycleProgressOverride = Math.max(0, Math.min(1, numeric));
    return this.nightCycleProgressOverride;
  }
  ...
  updateFogColorForTime(elapsedTimeSeconds = 0) {
    ...
    let t;
    if (Number.isFinite(this.nightCycleProgressOverride)) {
      t = Math.max(0, Math.min(1, this.nightCycleProgressOverride));
    } else {
      const duration = Math.max(0.001, this.nightTransitionDuration);
      const clampedTime = Math.max(0, Number(elapsedTimeSeconds) || 0);
      t = Math.min(1, clampedTime / duration);
    }
    for (let i = 0; i < 4; i += 1) {
  ...
  render(world) {
    const gl = this.gl;
    const fogNearUniform = this.fogEnabled ? this.fogNear : 1.0e9;
    const fogFarUniform = this.fogEnabled ? this.fogFar : 1.0e9 + 1.0;

    this.updateFogColorForTime(world?.time ?? 0);
    this.handleView(world);
    gl.uniform4fv(this.u_fogColor, this.fogColor);
    gl.uniform1f(this.u_fogNear, fogNearUniform);
    gl.uniform1f(this.u_fogFar, fogFarUniform);
    ...
    this.renderInfectionOverlay(world);
    this.renderDreadChaseInfectionOverlay(world);
    this.renderInfectionParticles(world);
    this.renderWindLeafParticles(world);
  ...
  renderBlock(block) {
    ...
    this.cubeMesh.render(
      ...
      block.faceVisibility || null,
      block.frontFaceOnly === true,
      block.useFaceShading !== false
    );
  }
  ...
  renderEntity(entity) {
    ...
    if (entity.renderShape === "cube") {
      ...
      this.cubeMesh.render(
        ...
        entity.faceVisibility || null,
        entity.frontFaceOnly === true,
        entity.useFaceShading !== false
      );
      return;
    }
